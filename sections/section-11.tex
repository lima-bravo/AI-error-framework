...
\textbf{Why this matters:} A completely correct output can still cause real harm --- delivered to the wrong person, used past its intended scope, or relied on by someone without the standing to rely on it that way --- and none of that shows up in a check built only to catch wrong answers.

\textbf{Practical implication:} Before checking whether outputs are correct, name the specific losses that must not happen for this job, and work backward to what would have to fail for each one to occur.
